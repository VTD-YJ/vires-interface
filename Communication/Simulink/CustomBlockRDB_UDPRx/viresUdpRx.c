...
*/
static void openUDPPort( unsigned int portId );
static unsigned int readUDPPort( void );
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    unsigned char* inAddr00      = (unsigned char*)ssGetInputPortSignal( S, 4 );
    unsigned char* inAddr01      = (unsigned char*)ssGetInputPortSignal( S, 5 );
    unsigned char* inAddr02      = (unsigned char*)ssGetInputPortSignal( S, 6 );
    unsigned char* inAddr03      = (unsigned char*)ssGetInputPortSignal( S, 7 );
    ...
    // compute the network data size;
    sNetworkDataSize = sNetworkStreamNoElements * sNetworkStreamElementSize;
    ...
    // open the UDP Port
    openUDPPort( *inPortNo );

    // read the port
    *outDataSize = readUDPPort();
    
    // reset the output data
    memset( outData, 0, sNetworkDataSize );
    
    if ( *outDataSize && ( *outDataSize <= sNetworkDataSize ) )
        memcpy( outData, mRxBuffer, *outDataSize );
...
static void openUDPPort( unsigned int portId )
{
    ...
    if ( mDescriptor == -1 )
    {
        printf( "openUDPPort: could not open socket\n" );
        return;
    }
    ...
    {
        printf( "open: could not set address to reuse\n" );
        return;
    }
    ...
    if ( bind( mDescriptor, (struct sockaddr *) &sock, sizeof( struct sockaddr_in ) ) == -1 )
    {
        printf( "open: could not bind socket\n" );
        perror( "reason: " );
        return;
    }
...
static unsigned int readUDPPort( void )
{
    ...
    // buffer not yet defined?
    if ( !mRxBuffer )
        mRxBuffer = ( char* ) calloc( 1, sNetworkDataSize );
        
    if ( !mRxBuffer )
    {
        printf( "open: could not allocate receive buffer\n" );
        return 0;
    }
        
    int noBytes = recv( mDescriptor, mRxBuffer, sNetworkDataSize, 0 );
